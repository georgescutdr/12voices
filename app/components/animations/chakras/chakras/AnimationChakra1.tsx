...
export default function AnimationChakra1() {
  ...
  useEffect(() => {
    ...
    // Resize canvas to fit container size exactly
    const setSize = () => {
      ...
      // Set canvas pixel buffer size accounting for DPR
      canvas.width = W * dpr;
      canvas.height = H * dpr;
    ...
    // Chakra config uses container-based W and H now
    const chakra = {
      x: () => W / 2,
      y: () => H / 2,
      petals: 4,
    ...
    const drawChakra = (ctx: CanvasRenderingContext2D, chakra: typeof chakra) => {
      ctx.save();
    ...
    const animate = () => {
      ...
      // Fill background white (you can customize this)
    ...
    >
      <canvas
        ...
        style={{
          ...
          width: '100%',
          height: '100%',
          backgroundColor: 'transparent', // transparent since parent is white here
        }}
      />
    </div>
  );
